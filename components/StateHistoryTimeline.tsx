  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-8">
        ...
        <span className="ml-3 text-brand-gray-400">Loading history...</span>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-brand-gray-400">Current:</span>
          <JobStatusBadge status={currentState} size="sm" />
        </div>
      </div>

      <div className="relative">
        <div className="absolute left-4 top-0 bottom-0 w-0.5 bg-brand-gray-700"></div>
            ...
            <div key={transition.id} className="relative pl-10">
              <div 
                className={`absolute left-2.5 w-3 h-3 rounded-full border-2 ${
                  index === 0 ? 'bg-brand-cyan-500 border-brand-cyan-400' : 'bg-brand-gray-600 border-brand-gray-500'
                }`}
                style={{ top: '0.5rem' }}
              ></div>
              ...
              <div className="bg-brand-gray-800 rounded-lg p-4 border border-brand-gray-700">
                ...
                {transition.reason && (
                  <div className="mt-2">
                    <p className="text-sm font-medium text-brand-gray-300">Reason:</p>
                    <p className="text-sm text-brand-gray-400">{transition.reason}</p>
                  </div>
                )}

                {transition.notes && (
                  <div className="mt-2">
                    <p className="text-sm font-medium text-brand-gray-300">Notes:</p>
                    <p className="text-sm text-brand-gray-400">{transition.notes}</p>
                  </div>
                )}

                <div className="mt-3 flex items-center gap-4 text-xs text-brand-gray-500">
                  {transition.changedBy && (
                    <span>
                      By: <span className="text-brand-gray-400">{transition.changedBy}</span>
                  ...
                  <span>
                    Source: <span className="text-brand-gray-400">{formatChangeSource(transition.changeSource)}</span>
                  </span>
                </div>

                {transition.metadata && Object.keys(transition.metadata).length > 0 && (
                  <details className="mt-3">
                    <summary className="text-xs text-brand-gray-500 cursor-pointer hover:text-brand-gray-400">
                      View metadata
                    </summary>
                    <pre className="mt-2 text-xs text-brand-gray-400 bg-brand-gray-900 p-2 rounded overflow-x-auto">
