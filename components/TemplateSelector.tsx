        ...
        <div className="p-6 space-y-4">
          <div className="flex flex-col sm:flex-row gap-4">
            <input
              ...
              className="flex-1 px-4 py-2 bg-brand-gray-800 border border-brand-gray-600 rounded-md text-white placeholder-brand-gray-400 focus:outline-none focus:border-brand-cyan-500 focus:ring-1 focus:ring-brand-cyan-500"
            />
            <select
              value={selectedCategory}
              onChange={(e) => setSelectedCategory(e.target.value)}
              className="px-4 py-2 bg-brand-gray-800 border border-brand-gray-600 rounded-md text-white focus:outline-none focus:border-brand-cyan-500 focus:ring-1 focus:ring-brand-cyan-500"
            >
              {categories.map((cat) => (
          ...
          <div className="overflow-y-auto max-h-[60vh]">
            {isLoading ? (
              <div className="flex items-center justify-center py-12">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-brand-cyan-500"></div>
                <span className="ml-3 text-brand-gray-400">Loading templates...</span>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredTemplates.map((template) => (
                  <button
                    ...
                    className="text-left p-4 bg-brand-gray-800 border border-brand-gray-700 rounded-lg hover:border-brand-cyan-500 hover:bg-gray-750 transition-colors"
                  >
                    ...
                    {template.description && (
                      <p className="text-sm text-brand-gray-400 mb-3 line-clamp-2">{template.description}</p>
                    )}
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      {template.basePrice && (
                        <div className="text-brand-gray-300">
                          <span className="text-brand-gray-500">Base: </span>${template.basePrice.toFixed(2)}
                        </div>
                      )}
                      {template.defaultDurationHours && (
                        <div className="text-brand-gray-300">
                          <span className="text-brand-gray-500">Duration: </span>{template.defaultDurationHours}h
                        </div>
                      )}
                      {template.defaultCrewSize && (
                        <div className="text-brand-gray-300">
                          <span className="text-brand-gray-500">Crew: </span>{template.defaultCrewSize} people
                        </div>
                      )}
                      {template.usageCount > 0 && (
                        <div className="text-brand-gray-300">
                          <span className="text-brand-gray-500">Used: </span>{template.usageCount} times
                        </div>
                      )}
                    </div>
