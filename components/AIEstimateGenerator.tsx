...
const AIEstimateGenerator: React.FC<AIEstimateGeneratorProps> = ({ customers, setQuotes }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow">
            ...
            {estimate && !isLoading && (<div className="mt-8 border-t border-brand-navy-200 pt-6"><h3 className="text-lg font-medium leading-6 text-brand-navy-900">AI Generated Estimate</h3><div className="mt-4 grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2"><div className="sm:col-span-1"><dt className="text-sm font-medium text-brand-navy-500">Confidence</dt><dd className="mt-1 text-lg text-brand-navy-900">{(estimate.confidence * 100).toFixed(0)}%</dd></div><div className="sm:col-span-1"><dt className="text-sm font-medium text-brand-navy-500">Difficulty</dt><dd className="mt-1 text-lg text-brand-navy-900">{estimate.difficulty}</dd></div><div className="sm:col-span-2"><dt className="text-sm font-medium text-brand-navy-500">Rationale</dt><dd className="mt-1 text-sm text-brand-navy-900">{estimate.rationale}</dd></div><div className="sm:col-span-2"><dt className="text-sm font-medium text-brand-navy-500">Editable Line Items</dt><dd className="mt-2"><div className="overflow-x-auto rounded-md border"><table className="min-w-full divide-y divide-brand-navy-200"><thead className="bg-brand-navy-50"><tr><th className="px-3 py-2 text-left text-xs font-medium text-brand-navy-500 uppercase tracking-wider">Description</th><th className="w-20 px-3 py-2 text-left text-xs font-medium text-brand-navy-500 uppercase tracking-wider">Qty</th><th className="w-28 px-3 py-2 text-left text-xs font-medium text-brand-navy-500 uppercase tracking-wider">Unit Price</th><th className="w-28 px-3 py-2 text-left text-xs font-medium text-brand-navy-500 uppercase tracking-wider">Total</th><th className="w-12 relative px-3 py-2"><span className="sr-only">Remove</span></th></tr></thead><tbody className="bg-white divide-y divide-brand-navy-200">{editableLineItems.map((item, index) => (<tr key={index}><td><input type="text" value={item.desc} onChange={(e) => handleItemChange(index, 'desc', e.target.value)} className="w-full border-0 border-b border-transparent focus:border-brand-cyan-500 focus:ring-0 sm:text-sm" /></td><td><input type="number" value={item.qty} onChange={(e) => handleItemChange(index, 'qty', e.target.value)} className="w-full border-0 border-b border-transparent focus:border-brand-cyan-500 focus:ring-0 sm:text-sm" /></td><td><input type="number" value={item.unit_price} onChange={(e) => handleItemChange(index, 'unit_price', e.target.value)} className="w-full border-0 border-b border-transparent focus:border-brand-cyan-500 focus:ring-0 sm:text-sm" /></td><td className="px-3 py-2 text-sm text-brand-navy-900">${(item.qty * item.unit_price).toFixed(2)}</td><td><button onClick={() => handleRemoveItem(index)} className="text-red-500 hover:text-red-700 p-1">X</button></td></tr>))}</tbody><tfoot><tr className="bg-brand-navy-50"><td colSpan={3} className="px-3 py-2 text-right text-sm font-bold text-brand-navy-900">Total</td><td className="px-3 py-2 text-sm font-bold text-brand-navy-900">${totalPrice.toFixed(2)}</td><td></td></tr></tfoot></table></div><button type="button" onClick={handleAddItem} className="mt-2 rounded-md bg-brand-navy-100 px-3 py-1.5 text-sm font-semibold text-brand-navy-900 shadow-sm ring-1 ring-inset ring-brand-navy-300 hover:bg-brand-navy-200">+ Add Item</button></dd></div></div><div className="mt-6 pt-6 border-t border-brand-navy-200 sm:flex sm:items-center sm:justify-between"><div><label htmlFor="customer-select" className="block text-sm font-medium text-brand-navy-700">Assign to Customer</label><select id="customer-select" value={selectedCustomerId} onChange={e => setSelectedCustomerId(e.target.value)} className="mt-1 block w-full sm:w-64 rounded-md border-brand-navy-300 shadow-sm focus:border-brand-cyan-500 focus:ring-brand-cyan-500 sm:text-sm">{customers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div><div className="mt-4 sm:mt-0"><button type="button" onClick={handleSaveAsQuote} className="inline-flex w-full justify-center items-center rounded-md border border-transparent bg-green-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">Save as Draft Quote</button></div></div></div>)}
        </div>
    );
};
