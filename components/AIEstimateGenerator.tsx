...
const AIEstimateGenerator: React.FC<AIEstimateGeneratorProps> = ({ customers, setQuotes }) => {
    ...
    const handleSaveAsQuote = () => {
        if (!estimate || !selectedCustomerId) {
            alert("Cannot save quote. Missing estimate data or customer selection.");
            return;
        }
        ...
        const newQuote: Quote = {
            id: `quote-${Date.now()}`,
            leadId: '', // This would be linked in a more complex flow
        ...
        alert(`Draft quote #${newQuote.id} for ${customer.name} created successfully!`);
        // Reset form
        setEstimate(null);
        setImages([]);
        setDescription('');
        ...
        <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                <div className="sm:col-span-6">
                    <label htmlFor="job-description" className="block text-sm font-medium text-brand-navy-700">Job Description</label>
                    <textarea id="job-description" name="job-description" rows={4} className="mt-1 block w-full rounded-md border-brand-navy-300 shadow-sm focus:border-brand-cyan-500 focus:ring-brand-cyan-500 sm:text-sm" placeholder="e.g., 'Large oak tree in the backyard, close to the fence...'" value={description} onChange={(e) => setDescription(e.target.value)} />
                </div>

                <div className="sm:col-span-6">
                    <label htmlFor="file-upload" className="block text-sm font-medium text-brand-navy-700">Upload Photos</label>
                    <div className="mt-1 flex justify-center rounded-md border-2 border-dashed border-brand-navy-300 px-6 pt-5 pb-6">
                        <div className="space-y-1 text-center">
                            <svg className="mx-auto h-12 w-12 text-brand-navy-400" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true"><path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
                            <div className="flex text-sm text-brand-navy-600"><label htmlFor="file-upload" className="relative cursor-pointer rounded-md bg-white font-medium text-brand-cyan-600 focus-within:outline-none focus-within:ring-2 focus-within:ring-brand-cyan-500 focus-within:ring-offset-2 hover:text-brand-cyan-500"><span>Upload files</span><input id="file-upload" name="file-upload" type="file" className="sr-only" multiple onChange={handleImageChange} accept="image/*" /></label><p className="pl-1">or drag and drop</p></div>
                            <p className="text-xs text-brand-navy-500">PNG, JPG, GIF up to 10MB</p>
                        </div>
                    </div>
                    {images.length > 0 && <div className="mt-2 text-sm text-brand-navy-500">{images.length} file(s) selected: {images.map(f => f.name).join(', ')}</div>}
                </div>
            </div>
