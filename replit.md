...
- **Voice Recognition**: Production-ready continuous listening implementation in `useVoiceRecognition.ts`. Features: (1) **Continuous Mode** - never stops/restarts between wake→command→wake transitions for smooth UX, (2) **Wake Word Detection** - accurately detects "Yo Probot" and transitions to command mode, (3) **Command Accumulation** - properly accumulates multi-word commands by appending new final transcripts instead of overwriting, (4) **isActiveRef Pattern** - clean on/off control that prevents unwanted auto-restarts, (5) **Wake Word Removal** - strips "yo probot" from first command chunk to avoid sending it to AI.
