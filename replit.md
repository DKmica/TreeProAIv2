...
### UI/UX Decisions
The application features a modern, dark theme with bright cyan (#00c2ff) accents against a dark navy/gray background (#0a1628 to #102a43). It includes a custom "futuristic AI circuit tree" logo and consistent branding across all UI components. The UI is designed to be fully mobile-responsive across all pages and forms, with tables converting to card layouts on mobile devices and modals adapting to small screens.

### Technical Implementations
- **Frontend**: Built with React 19, TypeScript, and Vite, using TailwindCSS for styling and React Router DOM for navigation.
- **Backend**: Developed using Node.js and Express, providing a RESTful API.
- **Database**: PostgreSQL 14+ is used.
...
- **AI-Powered Estimating**: Uses Gemini for detailed estimates, including pricing and suggested services, with user feedback and learning mechanisms.
...
- **Operations & Scheduling**: Includes crew management, time tracking, atomic job assignment with conflict detection, calendar integration, and dynamic job forms system.
- **Invoice Management System**: Professional invoicing with comprehensive financial tracking, Stripe payment processing, auto-invoice creation from completed jobs, and webhook-driven payment status updates.
- **Company Settings**: Comprehensive business information management with sections for contact, address, business details, and legal information.
- **Automation Engine**: Rule-based workflow automation with event-driven triggers, CRON scheduling, condition evaluation, and actions (send email/SMS, create tasks, update statuses, webhooks), including pre-built templates and execution logging.
- **Quoting & Proposal System**: Enhanced quoting with professional proposals, version history, e-signature capture, multi-tier pricing, and conversion analytics.
- **Advanced Scheduling**: Features route optimization, real-time crew tracking, skill-based crew matching, conflict resolution, and customer notifications.

### System Design Choices
- **Modular Structure**: AI functionalities and other domains are modularized into distinct services (Router-Controller-Service pattern).
...
- **Testing Infrastructure**: Comprehensive testing setup with Vitest, Testing Library, Playwright, and Supertest for unit, integration, and E2E tests.
- **Payment Processing**: Stripe integration with webhook-driven invoice status updates, secure credential caching, and transaction-safe processing.
- **Invoice Automation**: Auto-generated invoices on job completion with sequential numbering.
- **Production Deployment**: Backend serves static frontend files, with deployment target `autoscale`.
