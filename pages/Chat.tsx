...
import { useAiCoreStatus, useInitializeAiCore } from '../contexts/AppDataContext';

const ChatPage: React.FC = () => {
    const isAiCoreInitialized = useAiCoreStatus();
    const initializeAiCore = useInitializeAiCore();
    
    useEffect(() => {
        if (!isAiCoreInitialized) {
            initializeAiCore();
        }
    }, [isAiCoreInitialized, initializeAiCore]);
                ...
                <div className="border-t border-brand-gray-200 p-4 bg-white rounded-b-lg">
                    <form onSubmit={handleSubmit} className="flex items-center space-x-3">
                        ...
                        <input
                            ...
                            className="flex-1 block w-full rounded-md border border-brand-gray-600 bg-brand-gray-800 px-3 py-2 text-white shadow-sm placeholder:text-brand-gray-400 focus:border-brand-cyan-500 focus:ring-1 focus:ring-brand-cyan-500 sm:text-sm"
